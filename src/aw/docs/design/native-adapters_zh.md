...
2. `Adapter::capture` 将可观测的原生身份与上下文逐项比对，核对运行代次和会话，再提取一个 UTF-8 文本位置。摘要覆盖原始字节，保留空结果和空白。解码后的原始 payload 完整保留，包括 AW 合同之外的原生字段。
...
示例使用 Qoder 格式的合成输入、实际适配层和 Core、**合成 Provider**，以及 `FileJournal`。它重新打开并校验日志，确认原始 payload 未变，然后删除 `target` 下的临时目录。示例不启动 Agent，也不调用 SecCore 扫描器。测试覆盖六类字段映射、身份错配、计划绑定、Provider 失败、重复事件和前置拒绝。核验环境为 Linux ARM64；这些测试没有验收真实框架启用或其他操作系统。
