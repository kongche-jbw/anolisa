...
impl Core {
    ...
    pub fn execute(
        ...
        let journal_ack = journal.append(
            &event_key,
            &json!({
                "kind": "execution_settled", "execution": execution
            }),
        )?;
        // A custom journal must return a well-shaped reference before callers
        // receive a usable terminal result. Its durability is a trusted port duty.
        let mut acknowledged = execution.clone();
        acknowledged["evidence"] = Value::Array(vec![journal_ack.clone()]);
        self.registry.validate("plan-execution-v1", &acknowledged)?;
        Ok(Execution {
            plan,
            boundary,
            execution,
