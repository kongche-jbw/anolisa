...
///
/// This launches only native version probes, with the declared environment,
/// budgets and cancellation. It creates no Agent, hook, Journal or adoption
/// record. Success is a dependency snapshot, not runtime or security readiness;
/// execution still performs its own admission and identity checks.
///
/// # Errors
/// Returns the responsible Host's version/pin/process failure. Inspection does
/// not probe Tokenless; projection requires both and stops on SecCore failure.
pub fn run(
    settings: Settings,
    ...
    SecHost::new_cancellable(security, cancellation.clone())?;
