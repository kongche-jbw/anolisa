...
#[test]
fn qoder_completed_bash_rejects_contradictory_failure_markers() {
    let (event, mut payload) = fixture(Host::Qoder, false);
    payload["tool_response"] = json!({"kind":"completed","stdout":"text","stderr":"",
        "exitCode":0,"signal":null,"interrupted":false,"isImage":false,"noOutputExpected":false});
    for (field, value) in [
        ("error", json!("failed")),
    ...
    payload["tool_response"]["error"] = Value::Null;
    payload["tool_response"]["isError"] = json!(false);
    assert_eq!(extract(Host::Qoder, event, &payload).unwrap().text, "text");
}
