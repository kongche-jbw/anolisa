...
impl Adapter {
    ...
    /// Capture cross-checks identities and the artifact shape; complete scope and
    /// plan validation occurs during `prepare`, before any Provider invocation.
    ///
    /// # Errors
    ...
    pub fn capture(&self, request: CaptureRequest) -> Result<CapturedEvent, Error> {
        ...
        let boundary = profiles::descriptor(&self.profile, &native_event)?;
