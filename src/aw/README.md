...
After setup, run `src/aw/scripts/cosh --allow-unrecoverable` from the clone root. This starts the compiled Rust cosh-shell. Ordinary commands stay in its native shell; typing `qoder` or `codex` attaches Herdr, and exit returns to the same shell. Qoder connects AW; Codex currently attaches only its terminal and displays that AW hooks are not connected. `Ctrl+B p` opens Provider details with clickable tabs, wheel scrolling, actual paths, and scanner findings. See the guide for a synthetic API-key detection example.
