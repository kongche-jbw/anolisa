...
After setup, run `src/aw/scripts/cosh --allow-unrecoverable` from the clone root. This starts the compiled Rust cosh-shell. Ordinary commands stay in its native shell; typing `qoder` or `codex` attaches Herdr, and exit returns to the same shell. Qoder connects inspection and projection; Codex connects native SecCore inspection after first-use trust in `/hooks`. Codex explicitly marks Tokenless replacement unsupported. `Ctrl+B p` opens Provider details with clickable tabs, wheel scrolling, actual paths, and scanner findings. See the guide for a synthetic API-key detection example.

Herdr split panes use independent AW sessions for both `qoder` and `qodercli`. Provider details follow the focused pane, and an Agent exit preserves other panes. Start a fresh Herdr instance after updating; existing processes keep their original hook configuration.
