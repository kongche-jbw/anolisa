...
pub fn handle(args: EnvArgs, ctx: &CliContext) -> Result<(), CliError> {
    ...
    println!("libc: {}", display_opt_str(facts.libc.as_deref()));
    println!("kernel: {}", display_opt_str(facts.kernel.as_deref()));
    println!("pkg_base: {}", display_opt_str(facts.pkg_base.as_deref()));
    println!("btf: {}", display_opt_bool(facts.btf));
    println!("cap_bpf: {}", display_opt_bool(facts.cap_bpf));
    println!("container: {}", display_opt_str(facts.container.as_deref()));
    println!("user: {}", facts.user);
    println!("uid: {}", facts.uid);
