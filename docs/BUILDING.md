...
5. **Sandbox policy hooks are not enabled automatically.** After installation, run `/hooks install` inside Copilot Shell once to activate the built-in sandbox-guard hooks. See [3.3 Post-Installation: Enable Sandbox Hooks](#3.3-post-installation-enable-sandbox-hooks) below.
